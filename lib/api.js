...
async function call(method, path, body, opts = {}) {
  ...
  if (!res.ok) {
    /* ==================================================================
       SOMEBODY ELSE'S TEXT IS PRINTED AS TEXT, NOT AS COMMANDS TO THE
       TERMINAL.

       The refusal message comes from the marketplace, and it went to the
       terminal byte for byte — ANSI control sequences and carriage returns
       included, if there were any in it. It is somebody else's string on
       your screen: with it the output can be recoloured, what is already
       written can be erased, and the tool's own text can be hidden. The
       control characters are stripped, the newline is kept, and the length
       is cut. */
