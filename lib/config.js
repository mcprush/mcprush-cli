...
export function readClientFile(client) {
  ...
  try {
    ...
    /* IT PARSED DOES NOT YET MAKE IT A CONFIG. `[]`, `"text"` and `5` are
       valid JSON, and on those a raw TypeError fell out further down instead
       of a sentence. */
...
/* ==========================================================================
   WHAT WE WRITE INTO HAS TO BE AN OBJECT — NOT SOMETHING THAT RESEMBLES ONE.

   The check was `typeof node[k] !== 'object'`, and an array passed it: typeof
   [] is 'object'. Further along, the write `bucket[<key>] = {...}` put a
   NAMED property on an array, JSON.stringify threw it away, and the tool
   printed "✓ entry added", recorded the install on the account and returned
   ok:true — with an empty file. The worst of all possible outcomes: a success
   that never happened.

   An array where mcpServers belongs is not our "the key is not there yet"
   case but somebody else's file, arranged differently from what we assume.
   This file's rule on that is already written two functions below: a config
   we did not understand is not rewritten. */
...
export function skillDirFor(clientId, folder, opts = {}) {
  ...
  /* AND THE FIRST SEGMENT ITSELF CAN BE A LINK. The assembled path was
     checked as a whole, but `.claude` (or `.cursor`, `.agents`) is a
     directory of its own that anybody could have turned into a symlink
     pointing outward beforehand: then "inside the project" meant "into
     somebody else's place", and both the file writes and the recursive delete
     in `skill remove` went there. Every segment is checked as we descend, by
     real paths. */
  if (root) {
    ...
    for (const seg of parts) {
      ...
      /* lstat, not existsSync: a link to a target that has not been created
         yet "does not exist" as far as the latter is concerned, and the check
         would have missed exactly the case it stands here for. Once a segment
         is not there at all there is no point going deeper: the rest we
         create ourselves, already inside what has been checked. */
